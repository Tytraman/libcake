...
//Cake_AcceptedHttpClient *cake_http_server_accept(Cake_ServerSocket *serverSock, ulonglong requestMessageMaxLength) {
//    Cake_AcceptedHttpClient *client = (Cake_AcceptedHttpClient *) cake_new(sizeof(*client));
//    if(client == NULL)
//        return NULL;
//
//    client->sock = cake_server_socket_accept(serverSock);
...
//}
//
//cake_bool cake_http_add_header_element(Cake_HttpHeader **header, const char *key, const char *value) {
//    *header = (Cake_HttpHeader *) cake_new(sizeof(**header));
//    if(*header == NULL)
//        return cake_false;
...
//
//// TODO: codes d'erreur
//cake_bool __cake_http_receive(Cake_BytesBuffer *dest, Cake_BytesBuffer *destMessage, Cake_HttpHeader **header, cake_byte *getOrPost, Cake_String_UTF8 *url, HttpReceiveFusion *fusion, int (*recvCallback)(HttpReceiveFusion *, char *, int)) {
//    int bytesRead;
...
//    ulonglong index = 0;
//    while(1) {
//        if((bytesRead = recvCallback(fusion, buffer, CAKE_SOCKET_READ_BUFFER_SIZE)) == CAKE_HTTP_ERROR_RECV)
//            return cake_false;
//        else if(bytesRead == 0)
//            return cake_false;
//
//        dest->size += bytesRead;
//        ptr = cake_realloc(dest->buffer, dest->size * sizeof(*dest->buffer) + sizeof(*dest->buffer));
//        if(ptr == NULL)
//            return cake_false;
//        dest->buffer = (cake_byte *) ptr;
//        if(pass)
...
//            ptr = cake_realloc(dest->buffer, dest->size * sizeof(*dest->buffer) + sizeof(*dest->buffer) * 2);
//            if(ptr == NULL)
//                return cake_false;
//            dest->buffer = (cake_byte *) ptr;
...
//            destMessage->buffer = (cake_byte *) cake_new(destMessage->size * sizeof(*destMessage->buffer) + sizeof(*destMessage->buffer));
//            if(destMessage->buffer == NULL)
//                return cake_false;
...
//            if(remain > 0) {
//                while(1) {
//                    bytesRead = recvCallback(fusion, (char *) destMessage->buffer + current, (remain > CAKE_SOCKET_READ_BUFFER_SIZE ? CAKE_SOCKET_READ_BUFFER_SIZE : remain));
...
//}
//
//cake_bool cake_http_receive(Cake_BytesBuffer *dest, Cake_BytesBuffer *destMessage, Cake_HttpHeader **header, cake_byte *getOrPost, Cake_String_UTF8 *url, cake_socket sock) {
...
//            current = (Cake_LinkedList_String_UTF8_Pair *) cake_new(sizeof(*current));
//
//            if(b)
//                last->next = current;
//            else {
